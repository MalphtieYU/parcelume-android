...
- **本地加密保存**：敏感包裹字段使用 Android Keystore 中不可导出的密钥加密。
- **没有联网权限**：当前版本无法上传通知或包裹数据。
- **来源可控**：没有被用户选中的应用会在解析前直接忽略。
- **默认隐私模式**：默认不保存真实商品名称，用户可自行选择开启。
- **敏感页面拒绝**：付款、银行卡、密码和验证码页面不参与解析。
- **界面防窥**：包裹页面不会出现在截屏、录屏和最近任务预览中。
...
Parcelume 可能提取来源、可选商品标题、订单号、物流状态、运单号、取件码和更新时间。页面与通知原文只在内存中处理，不会写入数据库；付款或身份验证页面会被拒绝，敏感行会在提取前移除，保存的私人字段会在本机加密。识别服务不会点击、滚动、输入、执行手势或截屏。

## 下载测试版

从 [`v0.3.0-preview`](https://github.com/MalphtieYU/parcelume-android/releases/tag/v0.3.0-preview) 下载 `parcelume-0.3.0-debug.apk`。这是用于体验和验证的调试签名版本，不是应用商店正式版本；开启系统权限前请先阅读下方限制。
...
4. 首次勾选同意后点击“开启购物页面识别”。正常启动不会再次弹出，除非用户主动撤销同意，或未来读取范围发生实质变化。
...
- 用户主动确认后，同意状态只保存在本机；可以随时暂停全部识别或撤销同意。
...
| 静态数据保护 | 商品名称、订单号、运单号和取件码使用 Android Keystore 支持的随机 AES-GCM 加密；本地匹配使用带密钥指纹，不保留可搜索明文 |
| 敏感页面 | 付款、银行卡、密码和验证码页面直接拒绝；地址、联系方式、身份验证和支付相关行会在提取前移除 |
| 显示保护 | 使用 Android `FLAG_SECURE` 阻止应用截屏、录屏和最近任务预览 |
| 其他个人数据 | 不读取联系人、短信、照片、位置，不索取购物平台密码 |
...
0.3 已按明确 Android 包名支持 19 个应用：淘宝、京东、拼多多、天猫、菜鸟、顺丰、Amazon、Temu、AliExpress、eBay、Walmart、SHEIN、Etsy、Alibaba.com、Lazada、Shopee Indonesia、Flipkart、Mercado Libre 和日本乐天市场。
...
- 0.3 尚未接入实时快递查询，状态更新仍需要再次识别页面或通知。
